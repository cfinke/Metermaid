...
class METERMAID_METER {
	...
	public function __construct( $meter_id_or_row ) {
		...
		if ( is_numeric( $meter_id_or_row ) ) {
			$meter_data = wp_cache_get( $meter_id_or_row, 'metermaid-meter' );

			if ( false === $meter_data ) {
				$meter_data = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "metermaid_meters WHERE metermaid_meter_id=%s LIMIT 1", $meter_id_or_row ) );
				wp_cache_set( $meter_id_or_row, $meter_data, 'metermaid-meter' );
			}
	...
	public function gallons_ytd( $reading = null ) {
		...
		$reading_year = date( "Y", strtotime( $reading->reading_date ) );

		$january_1_reading = $this->estimate_real_reading_at_date( $reading_year . "-01-01" );

		return $reading->real_reading - ( (int) $january_1_reading );
	}
